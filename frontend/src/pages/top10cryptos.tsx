...
import { CoinModel } from "../../../backend/cryptopulse-api/src/models/CoinModel";
...
async function getTop10CryptoRanking(): Promise <any> {
    ...
    try {
        ...
        const url = `${server}/api/coins/get_top_10`;
        const rankingsData: any = await axios.get(url);
        if(rankingsData.data && rankingsData.data.data) {
            console.log("Top10 rankings: ", rankingsData.data.data);
...
//calls CommonUtils
export function invalidateTop10CryptosCache() {
...
export default function Top10Cryptos() {
   ...
   useEffect(() => {
      ...
      getData().then( (data) =>  {
        ...
        window.localStorage.setItem(TOP_10_IDS, JSON.stringify(data));
        eventBus.dispatch(eventBus.EVENTS.LOAD_TOP_10_COINMARKET, { message: "load top 10 cryptos", ids: data });
      });
   ...
   if(!top10 || !top10.length) {
    return (
        <div id="top_10_coins" className={`top_10_coins content-box ${styles.displayNone}`}>
            NO DATA!
        </div>
    )
   }
   return (
    ...
    <div id="top_10_coins" className={`top_10_coins content-box ${styles.displayNone} text-align-left`}>
        {getList()}
    </div>
