...
import { ethers, Interface, JsonRpcApiProvider, Log, Signer, ZeroAddress } from 'ethers'
...
import { getCurrentBalance, getDefaultChunkSize, getNetworkHeight, getTokenBalance, isReachableConnection, sleep } from '../utils/util.js'
import ERC20ABI from '../utils/ERC20ABI.json' assert {type: 'json'}
import { TokenTrackingData } from '../models/Tokens.js'
...
interface ThreadData {
    userId: string
    trackingData: TokenTrackingData // should be an array if is for the same chain
...
parentPort?.on('message', async (message) => {
    if (message.method === START_TRACKING_EVENTS.START_TRACKING_TOKEN) {
      ...
      try {
        token = await getExistingToken(userId, trackingData.token, trackingData.network)
      }catch(error) {
 ...
 */
async function getLastIndexedBlock(user: string, tokenAddress: string, chain: string): Promise <number> {
  const token = await getExistingToken(user, tokenAddress, chain)
  if(token) {
...
export async function processNetworkData(
    ...
    let lastIndexedBlock = await getLastIndexedBlock(userId, token,network) // trackingData.lastBlock || 0 // save on DB network -> last block processed for this wallet
    ...
    console.log(
      `RPC crawling details => Wallet address: ${token}, lastIndexedBlock: ${lastIndexedBlock}, Crawling start block: ${crawlingStartBlock}`
    )
    ...
    const doCrawl = async function() {
      ...
      if (!lockProccessing) {
        ...
        console.log(
          `Indexing network '${rpcDetails.network.chainName}', Token: ${token} , Last indexed block: ${lastIndexedBlock}, Start block: ${startBlock}, Network height: ${currentNetworkHeight}`
        )
          ...
          if(blocksToProcess > 0) {
            ...
            try {
              ...
              if(hasTransactions) {
                ...
                const updatedTransfers = await updateTokenTrackingDataWithTransferData(userId,trackingData.token,trackingData.network, blocksWithTransactionsFound)
                if(!updatedTransfers) {
    ...
    setInterval( async () => {
      const lastBlock = await getLastIndexedBlock(userId, token,network)
      if(lastBlock > -1) {
  ...
  export async function updateLastIndexedBlockNumber(
    ...
    lastIndexedBlock: number,
    trackingData:  TokenTrackingData,
    ...
    const updated = await updateExistingTokenBlock(userId, trackingData.token, trackingData.network, 0, lastProcessedBlock)
  ...
  export const processBlocks = async (
    ...
    try {
      ...
      if(blockLogs && blockLogs.length > 0) {
        events = await processChunkLogs(blockLogs, token, provider)
      } 
    ...
    if (logs.length > 0) {
      ...
      for (const log of logs) {
        ...
        if(event && event.type === EVENT_HASHES.Transfer.type) {
          ...
          if(txData) {
            const decodedTransaction = iface.parseTransaction({ data: txData, value: tx?.value })
          ...
          if(isTransactionIn || isTransactionOut || isPossibleMintOrBurn || isPossibleTokenContract) {
            const transactionData = await parseTransferEvent(event, log, provider, token)
            if(transactionData) {
...
async function parseTransferEvent(event: NetworkEvent, 
  log: ethers.Log, 
  provider: ethers.JsonRpcApiProvider, 
  token: string): Promise <ERC20Transfers | null> {
  ...
  if( txReceipt!=null) {
    ...
    if(decoded?.topic === EVENT_HASHES.Transfer.hash && decoded.name === EVENT_HASHES.Transfer.type && decoded?.args.length === 3) {
      ...
      let balanceAfter =  await contract.balanceOf(token)
      balanceAfter = Number(ethers.formatUnits(balanceAfter, decimals))
