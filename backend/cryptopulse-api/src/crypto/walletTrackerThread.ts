...
import { NetworkEvent, ProcessingEvents, RPCDetails } from "../types/common.js"
import { BlockchainTracking } from './blockchainTracking.js'
import { ERC20TransactionType, ERC20TransferDirection, ERC20Transfers } from '../models/ERC20TransferHistory.js'
import { ethers, Interface, isAddress, JsonRpcApiProvider, Log, Signer, ZeroAddress } from 'ethers'
import { CRAWLING_MESSAGES, EVENT_HASHES, START_TRACKING_EVENTS, TRANSACTION_EVENTS, TRANSACTION_SIGNATURE } from '../utils/Constants.js'
import { getCurrentBalance, getDefaultChunkSize, getNetworkHeight, isDefined, isReachableConnection, sleep } from '../utils/util.js'
import ERC20ABI from '../utils/ERC20ABI.json' assert {type: 'json'}
import { getExistingWallet, updateExistingWalletBlock, updateWalletTrackingDataWithTransferData } from '../wallets/index.js'
import { WalletTrackingData } from '../models/Wallets.js'
import { getCrawlingInterval, retryCrawlerWithDelay, crawlingInterval } from './trackingUtils.js'
import { WALLETS_LOGGER } from '../logging/common.js'

interface ThreadData {
    userId: string
    trackingData: WalletTrackingData // should be an array if is for the same chain
    rpcDetails: RPCDetails
}


const { userId, trackingData, rpcDetails } = workerData as ThreadData
...
parentPort?.on('message', async (message) => {
    if (message.method === START_TRACKING_EVENTS.START_TRACKING_WALLET) {

      try {
        wallet = await getExistingWallet(userId, trackingData.wallet, trackingData.network)
      }catch(error) {
        console.log('Unable to get wallet: ', trackingData.wallet)
        // TODO ABORT?
      }
      // start indexing the chain
      const blockchain = new BlockchainTracking(
        rpcDetails.rpc,
        rpcDetails.network.chainName,
        rpcDetails.network.chainId,
        rpcDetails.fallbackRPCs
      ) 
      const isReady = await (await blockchain.isNetworkReady()).ready
        // arrayOfListeners.push(blockChainHandler)
      if(isReady) {
        processNetworkData(blockchain, blockchain.getSigner())
      } else {
        const canStart = await retryCrawlerWithDelay(blockchain)
        if(canStart) {
          processNetworkData(blockchain, blockchain.getSigner())
        }
      }
 ...
 */
async function getLastIndexedBlock(user: string, address: string, chain: string): Promise <number> {
  const wallet = await getExistingWallet(user, address, chain)
  if(wallet) {
    return wallet.last_block ? wallet.last_block : -1
  }
  return -1
}
...
export async function processNetworkData(
    tracker: BlockchainTracking,
    // provider: JsonRpcApiProvider,
    signer: Signer
  ): Promise<void> {

    // isolate provider
    const provider: JsonRpcApiProvider = tracker.getProvider()

    // wallet to track
    const wallet = trackingData.wallet
    // network
    ...
    let lastIndexedBlock = await getLastIndexedBlock(userId, wallet,network) // trackingData.lastBlock || 0 // save on DB network -> last block processed for this wallet
    ...
    console.log(
      `RPC crawling details => Wallet address: ${wallet}, lastIndexedBlock: ${lastIndexedBlock}, Crawling start block: ${crawlingStartBlock}`
    )
  
    // we can override the default value of 30 secs, by setting process.env.INDEXER_INTERVAL
    let interval = getCrawlingInterval()
    let chunkSize: number = rpcDetails.chunkSize ? rpcDetails.chunkSize : 100 // parse 100 blocks at the time
    ...
    const doCrawl = async function() {
      ...
      if (!lockProccessing) {
        ...
        const currentNetworkHeight = await getNetworkHeight(provider)
        ...
        console.log(
          `Indexing network '${rpcDetails.network.chainName}', Wallet: ${wallet} , Last indexed block: ${lastIndexedBlock}, Start block: ${startBlock}, Network height: ${currentNetworkHeight}`
        )
        ...
        if (currentNetworkHeight >= startBlock && lastIndexedBlock < currentNetworkHeight) {
          ...
          try {
            console.log('try get balance before blocks for wallet: ', wallet)
            const balance = await getCurrentBalance(tracker.getProvider(), wallet)
            console.log(`balance before for wallet : ${wallet} ${balance}`)
            if(checkBalanceUpdates(trackingData.current_balance, balance)) {
              trackingData.current_balance = balance
            }
            // if(balance !== trackingData.current_balance) {
            //   console.log('BALANCE CHANGED 1, REPLICATE TRANSACTIONS FOR WALLET: ', wallet)
            //   const beforeData = structuredClone(trackingData)
            //   parentPort?.postMessage({
            //     method: TRANSACTION_EVENTS.FOUND_NATIVE_TRANSACTIONS,
            //     data: { balance_before: beforeData.current_balance, balance_after: balance }
            //   })
            //   trackingData.current_balance = balance
            // }

            // console.log('balance after: ', trackingData.currentBalance)
          }catch(ex) {
          ...
          if (!startedCrawling) {
            ...
            parentPort?.postMessage({
              method: CRAWLING_MESSAGES.TRACKING_WALLET_STARTED,
          ...
          if(blocksToProcess > 0) {
            ...
            try {
              ...
              console.log('try get balance AFTER blocks for wallet: ', wallet)
              const balance = await getCurrentBalance(tracker.getProvider(), wallet)
              console.log(`balance AFTER for wallet ${wallet} is: ${balance}`)
              ...
              console.log('while network height: ', await getNetworkHeight(provider))
              currentBlock = await updateLastIndexedBlockNumber(
                lastProcessedBlock, // the last on-chain block processed in this loop run
                lastIndexedBlock, // last block i have recorded on DB for this wallet
                trackingData,
                // blocksWithTransactionsFound
              )
              ...
              if(hasTransactions) {
                ...
                parentPort?.postMessage({
                  method: TRANSACTION_EVENTS.FOUND_WALLET_TRANSACTIONS,
                ...
                const updatedTransfers = await updateWalletTrackingDataWithTransferData(userId,trackingData.wallet,trackingData.network, blocksWithTransactionsFound)
                if(!updatedTransfers) {
    ...
    setInterval( async () => {
      const lastBlock = await getLastIndexedBlock(userId, wallet,network)
      if(lastBlock > -1) {
  ...
  export const retrieveChunkEvents = async (
    ...
    try {
      ...
      console.log('height: ', await getNetworkHeight(provider))
  ...
  export async function updateLastIndexedBlockNumber(
    ...
    lastIndexedBlock: number,
    trackingData: WalletTrackingData,
    // transfersData: ERC20Transfers[]
  ): Promise<number> {

    console.log('will updateLastIndexedBlockNumber to ', lastProcessedBlock)

    console.log(`will update last block for wallet ${trackingData.wallet} to ${lastProcessedBlock}` )
    const updated = await updateExistingWalletBlock(userId, trackingData.wallet, trackingData.network, trackingData.current_balance, lastProcessedBlock)
    console.log('updateExistingWalletBlock returned ', updated)
  ...
  export const processBlocks = async (
    ...
    try {
      let events: ERC20Transfers[] = []
      if(blockLogs && blockLogs.length > 0) {
        events = await processChunkLogs(blockLogs, wallet, network, provider)
      } 
      console.log('PROCESS BLOCKS FOR WALLET:', wallet)
  ...
  export const processChunkLogs = async (
    ...
    const storeEvents: ERC20Transfers[] = []
 
    if (logs.length > 0) {

      for (const log of logs) {
        WALLETS_LOGGER.info(`Checking event log: ${log}`)
        let contractAddress = log.address
        WALLETS_LOGGER.info(`contract address: ${contractAddress}`)
        ...
        if(event && event.type === EVENT_HASHES.Transfer.type) {
          ...
          WALLETS_LOGGER.info('checking if from/to matches wallet: '+ wallet)
          const tx = await provider.getTransaction(log.transactionHash)
          // WALLETS_LOGGER.info(`TX before: ${JSON.stringify(tx,null,4)}`)


          let transactionHash = tx?.hash
          WALLETS_LOGGER.info('transaction hash: ' + transactionHash)

          const toAddress = tx?.to
          const fromAddress = tx?.from

          let isTransactionOut = fromAddress === wallet
          let isTransactionIn = toAddress === wallet

          let isPossibleMintToWallet = !isTransactionIn && !isTransactionOut
          ...
          const isTransfer = isTransactionIn || isTransactionOut // txData && txData.indexOf(EVENT_HASHES.Transfer.method_id) > -1


          WALLETS_LOGGER.info('TRANSACTION OUT: ' + isTransactionOut)
          WALLETS_LOGGER.info('TRANSACTION IN: ' + isTransactionIn)
          WALLETS_LOGGER.info('POSSIBLE MINT ???: ' + isPossibleMintToWallet)

          if( (isTransfer && (isTransactionIn || isTransactionOut)) || isPossibleMintToWallet) {
            const transactionData = await parseTransferEvent(event, log, provider, contractAddress, wallet)
            if(transactionData) {
  ...
  export function findEventByHash(hashKeyToFind: string): NetworkEvent | null {
    for (const [key, value] of Object.entries(EVENT_HASHES)) {
      if (value.hash === hashKeyToFind) {
        return value as unknown as NetworkEvent
      }
    }
    return null
  }

  function checkBalanceUpdates(balanceBefore: number = 0, currentBalance: number = 0): boolean {
...
// async function parseMintoToAddressEvent(event: NetworkEvent, 
//   log: ethers.Log, 
...
//         balanceBefore = 0
//       }
//       WALLETS_LOGGER.info('balance before: ' + balanceBefore)
...
async function parseTransferEvent(event: NetworkEvent, 
  log: ethers.Log, 
  provider: ethers.JsonRpcApiProvider, 
  contractAddress: string, 
  wallet: string): Promise <ERC20Transfers | null> {
  ...
  if( txReceipt!=null) {
    ...
    if(!contractAddress && txReceipt.contractAddress) {
    ...
    if(decoded?.topic === EVENT_HASHES.Transfer.hash && decoded.name === EVENT_HASHES.Transfer.type && decoded?.args.length === 3) {
        const from = decoded.args[0]
        const to = decoded.args[1]

        if(from !== wallet && to!==wallet) {
          console.log('NOTHING RELATED WITH THIS WALLET, IGNORE')
          return null
        }
        // const signer = await provider.getSigner()
        const contract = new ethers.Contract(contractAddress, ERC20ABI, provider);
        ...
        else if(to === ZeroAddress && from === wallet) {
          isMint = false
          isBurn = true
          console.log('BURN CONFIRMED: ' + to)
          console.log('BURN TOKENS FROM WALLET: ' + from)
          direction = ERC20TransferDirection.DIRECTION_OUT
        }
        else {
          direction = to === wallet ? ERC20TransferDirection.DIRECTION_IN :  ERC20TransferDirection.DIRECTION_OUT
        }
        // const valueTransfer = BigNumber.from(decoded.args[2])
        WALLETS_LOGGER.info('VALUE: '+ decoded.args[2])
        const decimals = Number(await contract.decimals())
