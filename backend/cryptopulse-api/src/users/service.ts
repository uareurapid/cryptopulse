...
import { UserDBModel, WalletTrackingDBModel, WalletTransfersDBModel } from "../models/DBModels.js";
 ...
 */
export async function getUserById(user_id: string): Promise<UserResponse> { // the logged in user

  return await getUser(user_id, undefined)
  
}

export async function getUserByEmail(email: string): Promise<UserResponse> { // the logged in user

  return await getUser(undefined, email)
  
}

export async function getUser(user_id?: string, email?: string): Promise<UserResponse> { // the logged in user

  try {

    if( (!email && !user_id) || email?.length === user_id?.length && email?.length === 0) {
      console.log("getUser() - Invalid parameters, need one of (email,user_id)")
      return {}
    }
    console.log("getUser() - Will start getting info about user with user_id %s and email %s ", user_id, email)
    const collection = await CryptoPulseDatabase.getInstance().getCollection(TABLES.USERS)
    let filter = {}
    
    if(email && user_id) {
      filter = {email: email, user_id: user_id} 
    } else if(email) {
      filter = {email: email}
    }
    else {
    ...
    if(await collection.countDocuments(filter) === 0) {
        console.log('No user found with email: %s or user_id: %s', email, user_id) 
        return {} // TODO return empty array instead???
    }
    const userData = await collection.findOne(filter) as UserDBModel
