...
import { getCurrentBalance, getDefaultChunkSize, getNetworkHeight } from '../utils/util.js';
...
parentPort === null || parentPort === void 0 ? void 0 : parentPort.on('message', (message) => __awaiter(void 0, void 0, void 0, function* () {
    if (message.method === START_TRACKING_EVENTS.START_TRACKING_WALLET) {
        try {
            wallet = yield getExistingWallet(userId, trackingData.wallet, trackingData.network);
        }
        catch (error) {
 ...
 */
function getLastIndexedBlock(user, address, chain) {
    return __awaiter(this, void 0, void 0, function* () {
        const wallet = yield getExistingWallet(user, address, chain);
        if (wallet) {
...
signer) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let lastIndexedBlock = yield getLastIndexedBlock(userId, wallet, network); // trackingData.lastBlock || 0 // save on DB network -> last block processed for this wallet
        ...
        console.log(`RPC crawling details => Wallet address: ${wallet}, lastIndexedBlock: ${lastIndexedBlock}, Crawling start block: ${crawlingStartBlock}`);
        ...
        const doCrawl = function () {
            return __awaiter(this, void 0, void 0, function* () {
                if (!lockProccessing) {
                    ...
                    console.log(`Indexing network '${rpcDetails.network.chainName}', Wallet: ${wallet} , Last indexed block: ${lastIndexedBlock}, Start block: ${startBlock}, Network height: ${currentNetworkHeight}`);
                    ...
                    if (currentNetworkHeight >= startBlock && lastIndexedBlock < currentNetworkHeight) {
                        try {
                            ...
                            const balance = yield getCurrentBalance(tracker.getProvider(), wallet);
                        ...
                        if (blocksToProcess > 0) {
                            ...
                            try {
                                const processedBlocks = yield processBlocks(chunkEvents, wallet, network, provider, startBlock, blocksToProcess);
                                ...
                                console.log('while network height: ', yield getNetworkHeight(provider));
                                ...
                                if (hasTransactions) {
                                    ...
                                    const updatedTransfers = yield updateWalletTrackingDataWithTransferData(userId, trackingData.wallet, trackingData.network, blocksWithTransactionsFound);
                                    if (!updatedTransfers) {
        ...
        setInterval(() => __awaiter(this, void 0, void 0, function* () {
            const lastBlock = yield getLastIndexedBlock(userId, wallet, network);
            if (lastBlock > -1) {
...
export function updateLastIndexedBlockNumber(lastProcessedBlock, lastIndexedBlock, trackingData) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const updated = yield updateExistingWalletBlock(userId, trackingData.wallet, trackingData.network, trackingData.current_balance, lastProcessedBlock);
        console.log('updateExistingWalletBlock returned ', updated);
        return updated ? lastProcessedBlock : -1;
    });
}
export const processBlocks = (blockLogs, wallet, network, provider, lastIndexedBlock, count) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        let events = [];
        if (blockLogs && blockLogs.length > 0) {
            events = yield processChunkLogs(blockLogs, wallet, network, provider);
        }
...
export const processChunkLogs = (logs, wallet, network, provider) => __awaiter(void 0, void 0, void 0, function* () {
    const storeEvents = [];
    if (logs.length > 0) {
        for (const log of logs) {
            ...
            if (event && event.type === EVENT_HASHES.Transfer.type) {
                ...
                if ((isTransfer && (isTransactionIn || isTransactionOut)) || isPossibleMintToWallet) {
                    const transactionData = yield parseTransferEvent(event, log, provider, contractAddress, wallet);
                    if (transactionData) {
...
//   return null
// }
function parseTransferEvent(event, log, provider, contractAddress, wallet) {
    return __awaiter(this, void 0, void 0, function* () {
        console.log('EVENT IS:', event);
        console.log(`-- Event type [ ${event.type} ] was triggered for transaction: ${log.transactionHash}`);
        const eventObj = {
        ...
        try {
            ...
            if (txReceipt != null) {
                ...
                if ((decoded === null || decoded === void 0 ? void 0 : decoded.topic) === EVENT_HASHES.Transfer.hash && decoded.name === EVENT_HASHES.Transfer.type && (decoded === null || decoded === void 0 ? void 0 : decoded.args.length) === 3) {
                    ...
                    console.log('SYMBOL: ' + tokenSymbol);
                    console.log(`txt receipt: ${txReceipt}`);
                    let balanceAfter = yield contract.balanceOf(wallet);
                    balanceAfter = Number(ethers.formatUnits(balanceAfter, decimals));
...
//# sourceMappingURL=walletTrackerThread.js.map