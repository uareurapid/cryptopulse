...
walletsRoutes.post(`${WALLETS_API_BASE_PATH}/track_wallet`, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const existingData = yield getTrackedWallets(userId); // {wallets: WalletTrackingData[]}
        console.log('Get existing tracking data response', existingData);
        let existingItems = existingData.wallets;
        ...
        if (existsWallet) {
            ...
            let newItem = { wallet: payload.data.wallet, network: payload.data.network };
            // get block number
            ...
            const block = yield getStartBlockFromNetworkHeight(supported);
            ...
            const response = yield updateOrCreateWalletTracking(payload.user_id, newItem);
            if (response) {
...
walletsRoutes.get(`${WALLETS_API_BASE_PATH}/tracked_wallets`, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const data = yield getTrackedWallets(user_id);
...
// }
walletsRoutes.get(`${WALLETS_API_BASE_PATH}/wallet_tokens`, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    // Wallet address
    const address = req.params.address;
    try {
        console.log("Getting wallet ERC20 tokens for wallet: ", address);
        let tokens = yield fetchWalletTokens(address); //fetchCryptoRankings();
        res.json(tokens);
    }
    catch (ex) {
        console.error("got error", ex);
...
function fetchWalletTokens(address) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let token of nonZeroBalances) {
            ...
            let balanceAsNumber = balance.toFixed(2);
            const data = {
...
walletsRoutes.post(`${WALLETS_API_BASE_PATH}/label_wallet`, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        console.log("got body:", req.body);
        /**
         * {
         ...
         */
        const payload = req.body; //wallet to get logs for
        const walletData = yield getExistingWallet(payload.user_id, payload.wallet, payload.network);
        if (!walletData) {
            res.status(404).send('That wallet does not exist!');
        }
        else {
            const update = yield updateWalletLabel(payload.user_id, payload.wallet, payload.network, payload.label);
